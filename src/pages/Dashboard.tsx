...
const Dashboard = () => {
  ...
  const submitReflections = async () => {
    ...
    try {
      ...
      if (!session) {
        toast.error("You must be logged in to submit reflections.");
        return;
      }

      // Prepare submissions for all principles
      const submissions = [];
      for (let principleIndex = 0; principleIndex < principles.length; principleIndex++) {
        const principle = principles[principleIndex];
        for (let questionIndex = 0; questionIndex < principle.questions.length; questionIndex++) {
          const answerKey = `${principleIndex}-${questionIndex}`;
          const response = answers[answerKey];
          
          if (response && response.trim()) {
            submissions.push({
              user_id: session.user.id,
              principle: principle.name,
              question: principle.questions[questionIndex],
      ...
      if (submissions.length === 0) {
        toast.error("No reflections to submit.");
        return;
      }
