...
import { Calendar, CheckCircle, XCircle, User, Search, BarChart3, TrendingUp } from 'lucide-react';
...
export const SuperAdminAttendanceManagement = () => {
  ...
  const loadStats = async () => {
    try {
      ...
      const absentCount = attendanceRecords.filter(record => !record.attended).length;
      const attendanceRate = attendanceRecords.length > 0 ? (presentCount / attendanceRecords.length) * 100 : 0;
      
      // Find top campus by attendance
      const campusStats: { [key: string]: { present: number; total: number } } = {};
      attendanceRecords.forEach(record => {
        const campusLead = campusLeads.find(lead => lead.id === record.user_id);
        if (campusLead) {
          if (!campusStats[campusLead.campus_name]) {
            campusStats[campusLead.campus_name] = { present: 0, total: 0 };
          }
          campusStats[campusLead.campus_name].total++;
          if (record.attended) {
            campusStats[campusLead.campus_name].present++;
          }
        }
      });
      
      const topCampus = Object.keys(campusStats).reduce((a, b) => 
        (campusStats[a].present / campusStats[a].total) > (campusStats[b].present / campusStats[b].total) ? a : b, 
        Object.keys(campusStats)[0] || ''
      );
  ...
  const handleBulkUpdate = async (attended: boolean) => {
    try {
      const selectedRecords = filteredRecords.filter(record => {
        const campusLead = campusLeads.find(lead => lead.id === record.user_id);
        return campusLead && (
          (selectedCampusLead === '' || selectedCampusLead === 'all' || record.user_id === selectedCampusLead) &&
          (selectedCampus === '' || selectedCampus === 'all' || campusLead.campus_name === selectedCampus) &&
      ...
      setAttendanceRecords(prev => prev.map(record => {
        ...
        return isSelected ? { ...record, attended } : record;
      }));
      
      toast.success(`Updated ${selectedRecords.length} attendance records`);
    } catch (error) {
  ...
  const filteredRecords = attendanceRecords.filter(record => {
    ...
    const matchesCampus = selectedCampus === '' || selectedCampus === 'all' || (campusLead && campusLead.campus_name === selectedCampus);
    const matchesDate = selectedDate === '' || record.date === selectedDate;
    
    return matchesSearch && matchesCampusLead && matchesCampus && matchesDate;
  });

  const uniqueCampuses = [...new Set(campusLeads.map(lead => lead.campus_name))];
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Attendance Management (Superadmin)</h2>
          <p className="text-muted-foreground">Manage attendance records across all campuses</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Top Campus</p>
                <p className="text-lg font-bold">{stats.topCampus}</p>
              </div>
